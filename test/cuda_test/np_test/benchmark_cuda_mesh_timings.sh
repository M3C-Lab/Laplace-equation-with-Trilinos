...
output_root="$project_root/test/cuda_test"
...
summary_file="$output_root/cuda_benchmark_summary.txt"
...
mesh_dirs=(
  "$mesh_root/nx=128_ny=128_np=1"
  "$mesh_root/nx=128_ny=128_np=2"
)
...
{
  echo "CUDA benchmark summary"
  echo "runs = $runs"
  echo "solver_tolerance = $solver_tol"
  echo "mpi_exec = $mpi_exec"
  echo "cases = nx=128_ny=128_np=1 nx=128_ny=128_np=2"
  echo
} > "$summary_file"
...
for mesh_dir in "${mesh_dirs[@]}"; do
  ...
  {
    echo "CUDA mesh benchmark report"
    echo "mesh = $mesh_name"
...
echo "CUDA benchmark reports written to $output_root"

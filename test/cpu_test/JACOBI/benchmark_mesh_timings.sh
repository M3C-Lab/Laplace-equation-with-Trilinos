...
output_root="$project_root/test/cpu_test/JACOBI"
...
case_nx_list="${CASE_NX_LIST:-128 256 512 1024}"
...
for mesh_dir in "${mesh_dirs[@]}"; do
  ...
  for run_id in $(seq 1 "$runs"); do
    ...
    output="$(MPI_RANKS="$np" SOLVER_TOL="$solver_tol" LAPLACE_FEM_PREC_TYPE="$prec_type" "$runscript" "$nx" "$ny" --benchmark 2>&1)"
